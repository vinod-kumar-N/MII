            ...
            var sampleArr = [];
             ...
             var finalTable= "";
                $.each(sample, function(i){
	                sampleArr.push(sample[i].split("]"));
                  
                })
                tablestart = "<tr><td>";
                        endTable = "</td></tr>";
                    for(j=0; j<=sampleArr.length-1; j++){
                        table = "";
                        testsample1 = sampleArr[j];
                        for(i=0; i<=testsample1.length-1; i++){
                        table += testsample1[i].trim() +"</td><td>";
                         }
                         finalTable += tablestart + table + endTable;
                    }
               
             document.getElementById("demo").innerHTML = finalTable;
